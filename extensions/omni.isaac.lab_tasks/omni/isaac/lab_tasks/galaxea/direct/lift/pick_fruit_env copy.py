...
class R1MultiFruitEnv(DirectRLEnv):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if not rospy.core.is_initialized():
            rospy.init_node('r1_multi_fruit_env', anonymous=True)
        self.pub_l = rospy.Publisher('/motion_target/target_pose_arm_left', PoseStamped, queue_size=10)
        self.pub_r = rospy.Publisher('/motion_target/target_pose_arm_right', PoseStamped, queue_size=10)

        # TF TransformListener
        self.listener = tf.TransformListener()

        # Placeholders for extracted messages
    ...
    def _compute_arm_joints(self, l_arm_actions: torch.Tensor, r_arm_actions: torch.Tensor):
        # 处理手臂动作并发布目标姿态
        l_ee_pose_curr, l_ee_quat_curr, r_ee_pose_curr, r_ee_quat_curr = self._compute_frame_pose()
        # print("l_ee_pose_curr: ", l_ee_pose_curr)
        # print("l_ee_quat_curr: ", l_ee_quat_curr)
        # print("r_ee_pose_curr: ", r_ee_pose_curr)
        # print("r_ee_quat_curr: ", r_ee_quat_curr)
        carrot = self.scene["object1"]
        target_position_right = carrot.data.body_pos_w.squeeze(0).cpu().numpy() 
        target_orientation_right = carrot.data.body_quat_w.squeeze(0).cpu().numpy()
        print("target_position_right: ", target_position_right)
        print("target_orientation_right: ", target_orientation_right)
        # target_position_left = l_ee_pose_curr.squeeze(0).cpu().numpy()
        # target_orientation_left = np.roll(l_ee_quat_curr.squeeze(0).cpu().numpy(), shift=-1)
        # target_position_right = r_ee_pose_curr.squeeze(0).cpu().numpy()
        # # print("target_position_right: ", target_position_right)
        # target_orientation_right = np.roll(r_ee_quat_curr.squeeze(0).cpu().numpy(), shift=-1)

        # 发布目标位置
        # self.pub_l.publish(self.pose_msg(target_position_left, target_orientation_left))
        # print('xx ', target_position_right, target_o)
        target_position_right, target_orientation_right = target_position_right.squeeze(0), target_orientation_right.squeeze(0)
        self.pub_r.publish(self.pose_msg(target_position_right, target_orientation_right))
        ...
        if self.extracted_msg_right is not None:
            ...
            print("right_arm_joint_pos_target: ", self.right_arm_joint_pos_target)
        ...
        print("right_arm_base_link_pos: ", right_arm_base_link_pos)
        ...
        self.pub_tf(br, "base_link", base_link_pos, base_link_quat)
        self.pub_tf(br, "left_arm_base_link", left_arm_base_link_pos, left_arm_base_link_quat)
        self.pub_tf(br, "right_arm_base_link", right_arm_base_link_pos, right_arm_base_link_quat)
        # self.pub_tf(br, "target_frame_left", target_position_left, target_orientation_left)
        self.pub_tf(br, "object1", target_position_right, target_orientation_right)
        tf_listener = tf.TransformListener()
        if tf_listener.canTransform("right_arm_base_link", "object1", rospy.Time().now()):
            (right_trans,right_rot) = tf_listener.lookupTransform('right_arm_base_link', 'object1', rospy.Time(0))

        # right_trans, right_rot = self.get_tf(self.listener, "right_arm_base_link", "object1")
        # left_trans, left_rot = self.get_tf(self.listener,"left_arm_base_link", "target_frame_left")
        ...
        if right_trans is None or right_rot is None or left_trans is None or left_rot is None:
            return
        # Publish the message
        self.pub_r.publish(self.pose_msg(right_trans, right_rot))
        # self.pub_l.publish(self.pose_msg(left_trans, left_rot))
    ...
    def get_tf(self,listener,parent_frame, child_frame):
        """获取TF变换"""
