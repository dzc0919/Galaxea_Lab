...
def run_simulator(sim: sim_utils.SimulationContext, scene: InteractiveScene):
    ...
    wheel_joint_value = np.array([0.2692006686470151, 0.32073197533142844, 0.2555014677061562])*150
    ...
    # 假设 wheel_joint_values 是动态更新的，这里初始化一个列表来存储每次的值
    wheel_joint_values_history = []
    ...
    while simulation_app.is_running():
        ...
        # 设置机器人关节目标位置
        # robot_pos = robot.data.joint_pos
        robot_vel = robot.data.joint_vel
        wheel_joint_values = robot_vel[0 , 4:7]
        # print("robot_pos: ", robot_pos)
        # print("robot_vel: ", robot_vel)
        print("Wheel motor joint values : ", wheel_joint_values)
        wheel_joint_values_history.append(wheel_joint_values.cpu().numpy())  # 转为 NumPy 格式以便绘图
        ...
        # print("target_position_steer: ", target_position_steer)
        ...
        #     wheel_joint_values_history_array = np.array(wheel_joint_values_history)  # 转为 NumPy 数组
        #     for i in range(wheel_joint_values_history_array.shape[1]):
        #         plt.plot(wheel_joint_values_history_array[:, i], label=f'Wheel motor joint {i+1}')
        #     plt.xlabel("Time Steps")
        #     plt.ylabel("Joint Velocity")
        #     plt.title("Wheel Motor Joint Velocities Over Time")
        #     plt.legend()
        #     plt.grid(True)
        #     plt.show()
def main():
