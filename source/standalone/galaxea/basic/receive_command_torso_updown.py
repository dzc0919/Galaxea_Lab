...
def run_simulator(sim: sim_utils.SimulationContext, scene: InteractiveScene):
    ...
    while simulation_app.is_running():
        ...
        if time <= down_time:  # 下降down_time
            ...
            break  # 停止运行
        torso_joint_position = min_torso_joint_value + (max_torso_joint_value - min_torso_joint_value)
        target_position_torso = torch.tensor(torso_joint_position,dtype=torch.float32, device="cuda:0")
        torso_joint_position[3] = 0.5 * np.pi  # 设置第四个关节的目标位置
        # robot_pos = robot.data.joint_pos
        # print("robot_pos: ", robot_pos)
        # torso_joint_values = robot_pos[0 , 0:4] *180.0 / np.pi 
        # print("troso_joint_values: ", torso_joint_values)
        robot.set_joint_position_target(target_position_torso, joint_ids=torso_entity_cfg.joint_ids)
        # torso_joint_values_history.append(torso_joint_values.cpu().numpy())  # 转为 NumPy 格式以便绘图
        ...
        #     torso_joint_values_history_array = np.array(torso_joint_values_history)  # 转为 NumPy 数组
        #     for i in range(torso_joint_values_history_array.shape[1]):
        #         plt.plot(torso_joint_values_history_array[:, i], label=f'torso motor joint {i+1}')
        #     plt.xlabel("Time Steps")
        #     plt.ylabel("Joint Position (degrees)")
        #     plt.title("Torso Motor Joint Positopn Over Time")
        #     plt.legend()
        #     plt.grid(True)
        #     plt.show()
def main():
