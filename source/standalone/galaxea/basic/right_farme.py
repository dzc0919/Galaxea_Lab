...
def run_simulator(sim: sim_utils.SimulationContext, scene: InteractiveScene):
    ...
    while simulation_app.is_running():
        ...
        diff_ik_controller.set_command(right_ik_commands)
        ...
        right_joint_pos_des = diff_ik_controller.compute(
        ...
        # Set target position for the left arm

        # Set target position for the right arm
        robot.set_joint_position_target(
            right_joint_pos_des, joint_ids=right_arm_entity_cfg.joint_ids
        )

        # Wait for the movement of the right arm to complete (if necessary)
        sim.step()  # Ensure the simulation progresses
