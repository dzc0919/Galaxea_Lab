...
# def run_simulator(sim: sim_utils.SimulationContext, scene: InteractiveScene):
    ...
    positions_commands = [
        ...
        ([0.3864, -0.3698, 1.1475], [1.0, 0.0, 0.0, 0.0], None),
        # ([0.3864, -0.3698, 1.1475], [0.5120, 0.0, 0.8590, 0.0], None),
        # ([0.3864, -0.3698, 1.0555], [0.5120, 0.0, 0.8590, 0.0], "c"),  # 关闭夹爪
        # ([0.3864, -0.3698, 1.3957], [0.5120, 0.0, 0.8590, 0.0], None),
        # ([0.3864, -0.1124, 1.3957], [0.5120, 0.0, 0.8590, 0.0], None),
        # ([0.3864, -0.5237, 1.1475], [1.0, 0.0, 0.0, 0.0], "o"),  # 开启夹爪
        ]
    ...
    while simulation_app.is_running():
        ...
        # if count % 200 == 0 and index < total_steps:
        # # 获取目标位置和朝向
        #     target_position_right, target_orientation_right = positions_commands[index][:2]
        ...
        #     index += 1  # 移动到下一个目标
        # target_position_right = np.array(target_position_right)
        # target_position_right = torch.tensor(target_position_right, dtype=torch.float32, device="cuda:0").squeeze(0)
        # target_orientation_right = np.array(target_orientation_right)
        # target_orientation_right = torch.tensor(target_orientation_right, dtype=torch.float32, device="cuda:0").squeeze(0)
        ...
        for event in pygame.event.get():
                    if event.type == pygame.QUIT:
                        ...
                        if event.key == pygame.K_g:
                            ...
                            record_pos, record_orien = target_frame_right.get_world_poses()
                            print("Target Position:", record_pos)
                            print("Target Orientation:", record_orien)
