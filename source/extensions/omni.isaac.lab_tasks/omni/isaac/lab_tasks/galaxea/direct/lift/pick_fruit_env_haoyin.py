...
class R1MultiFruitEnv(DirectRLEnv):
    ...
    def _compute_arm_joints_relaxik(self, l_arm_actions: torch.Tensor, r_arm_actions: torch.Tensor):
        print("r_arm_actions:", r_arm_actions)
        r_arm_actions = self._get_tf_from_ros(r_arm_actions[0,:3], r_arm_actions[0,3:])
        ...
        if self.extracted_msg_left is not None:
            print("left_arm_joint_pos_target:", self.extracted_msg_left)
            self.left_arm_joint_pos_target = torch.tensor(self.extracted_msg_left, device=self.device).view(1, -1)
        


    def _get_tf_from_ros(self, target_position_right, target_orientation_right):
        ...
        self._pub_tf(br, "base_link", base_link_pos, base_link_quat)
        self._pub_tf(br, "left_arm_base_link", left_arm_base_link_pos, left_arm_base_link_quat)
        self._pub_tf(br, "right_arm_base_link", right_arm_base_link_pos, right_arm_base_link_quat)
        ...
        self._pub_tf(br, "target_frame_right", target_position_right, target_orientation_right)
        tf_listener = tf.TransformListener()
        
        right_trans, right_rot = self._get_tf(tf_listener, "right_arm_base_link", "target_frame_right")
        # left_trans, left_rot = self._get_tf(tf_listener, "left_arm_base_link", "target_frame_left")
        breakpoint()
        print(right_trans, right_rot)
        right_pose_tensor = torch.tensor([right_trans, right_rot], device=self.device) if right_trans is not None else torch.cat((target_position_right, target_orientation_right)).view(1,-1)
        print("get transformed right pose from ros:", right_pose_tensor)
        return right_pose_tensor
    



    def _get_tf(self, listener, parent_frame, child_frame):
        try:
            (trans, rot) = listener.lookupTransform(parent_frame, child_frame, rospy.Time(1))
    ...
    def _pub_ee_to_relaxik(self, arm_actions: torch.Tensor, pub: rospy.Publisher):
        ...
        arm_pose_stamp.pose.orientation.x = arm_actions[0, 4].item()
        arm_pose_stamp.pose.orientation.y = arm_actions[0, 5].item()
        arm_pose_stamp.pose.orientation.z = arm_actions[0, 6].item()
        arm_pose_stamp.pose.orientation.w = arm_actions[0, 3].item()
        pub.publish(arm_pose_stamp)

    def _pub_tf(self, br, link_name, pos, quat):
        """发布TF变换"""
        t = TransformStamped()
